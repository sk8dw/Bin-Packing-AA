...
Dacă 3-Partition are soluție $\Rightarrow$ putem grupa elementele în $N$ triplete de sumă $C$ $\Rightarrow$ fiecare triplet intră într-un bin $\Rightarrow$ Bin Packing are soluție cu $K = N$.

Analog, dacă Bin Packing are soluție cu $K = N$ și capacitate $C$, iar suma totală este $N \cdot C$, atunci fiecare bin este umplut exact cu $C$ $\Rightarrow$ fiecare bin conține exact 3 elemente $\Rightarrow$ avem o soluție pentru 3-Partition.
...
\textbf{Bin Packing este NP-complet.}
