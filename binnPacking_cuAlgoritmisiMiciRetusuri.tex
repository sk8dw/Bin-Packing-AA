...
\begin{document}
	...
	\author{Enache Stephan-Cristian\and
		...
		Velicu Alexandru-Ștefan\inst{1}}
	%
	\authorrunning{A. Ravariu, C. Enache, A. Velicu}

	\institute{Universitatea Natională de Știință și Tehnologie Politehnica București \\
		\url {https://upb.ro}}
	%
	\maketitle
	%
	...
	\subsection{Euristici pentru Bin Packing}
	...
	Din acest motiv, folosim \emph{euristici} care oferă soluții bune, de obicei apropiate de optim.
	...
	\paragraph{\Large Raport de aproximație pentru Next-Fit}
	...
	\paragraph{\Large Limită inferioară pentru Next-Fit}
	...
	\subsubsection{Euristici utilizabile}

	Euristicile clasice de bin packing se aplică direct:
	...
	Programul implementează două categorii principale de algoritmi pentru problema \textit{Bin Packing}: algoritmi exacți bazați pe backtracking și algoritmi euristici (First Fit, Best Fit și variantele lor Decreasing).

	\subsection{Descrierea modului de funcționare}

	\subsubsection{Algoritmi de Backtracking}

	Algoritmii de backtracking explorează toate modurile posibile de a plasa pachetele în binuri. Fiecare stare a căutării este reprezentată prin structura:
	...
	Această structură reține poziția curentă în arborele de căutare:
	\begin{itemize}
		\item \texttt{item\_idx} indică pachetul curent care trebuie plasat,
		\item \texttt{bin\_idx} indică binul în care se încearcă plasarea.
	\end{itemize}

	\paragraph{\Large Plasarea unui pachet în binurile existente}
	...
	\textbf{Explicație:}
	Dacă \texttt{bin\_idx} este mai mic decât numărul de binuri existente, algoritmul încearcă să plaseze pachetul în binul respectiv.
	Dacă încape, se actualizează spațiul liber și se trece la următorul pachet.
	Dacă nu încape, se incrementează \texttt{bin\_idx} pentru a încerca următorul bin.

	\paragraph{\Large Deschiderea unui bin nou}
	...
	\textbf{Explicație:}
	Dacă toate binurile existente au fost încercate, algoritmul deschide un bin nou și plasează pachetul acolo.
	Apoi continuă cu următorul pachet.

	\paragraph{\Large Backtracking: revenirea la starea anterioară}

	\begin{verbatim}
		void undo_move(vector<bin>& bins, int bin_idx, int weight) {
			bins[bin_idx].space_left += weight;
			bins[bin_idx].packets.pop_back();
			if(bins[bin_idx].packets.empty()) {
				bins.pop_back();
			}
		}
	\end{verbatim}
	...
	\textbf{Explicație:}
	Această funcție inversează ultima mutare:
	\begin{itemize}
		\item readaugă greutatea în spațiul liber,
		\item elimină pachetul din bin,
		\item dacă binul a rămas gol, este eliminat complet.
	\end{itemize}

	\paragraph{\Large Backtracking asupra ultimului pachet}

	\begin{verbatim}
		void backtrack_last_item(vector<bin>& bins, vector<SearchState>& stack,
			 vector<packet> pack) {
	...
	\textbf{Explicație:}
	Când o ramură a căutării este epuizată, algoritmul:
	\begin{enumerate}
		\item scoate pachetul plasat anterior,
		\item incrementează \texttt{bin\_idx} pentru a încerca următoarea opțiune,
		\item continuă explorarea.
	\end{enumerate}


	\subsubsection{Algoritmi Euristici}

	\paragraph{\Large First Fit (FF)}
	...
	\textbf{Explicație:}
	Algoritmul parcurge binurile în ordine și plasează pachetul în primul bin în care încape.
	Este rapid, dar poate produce soluții suboptimale.

	\paragraph{\Large Best Fit (BF)}

	\begin{verbatim}
		if(bins[j].space_left > pack[i].weight &&
		bins[j].space_left - pack[i].weight < remaining_space_after_fit) {
			best_index = j;
	...
	\textbf{Explicație:}
	Algoritmul caută binul în care pachetul va lăsa cel mai puțin spațiu liber.
	Aceasta minimizează fragmentarea și produce soluții mai bune decât FF.

	\paragraph{\Large Variantele Decreasing}
	...
	\textbf{Explicație:}
	Sortarea descrescătoare reduce dramatic riscul de fragmentare.
	FFD și BFD sunt printre cele mai eficiente euristici clasice.
	...
	\begin{center}
		\begin{tabular}{|l|c|c|}
			\hline
		\textbf{Algoritm} & \textbf{Complexitate timp} & \textbf{Complexitate spațiu} \\
			\hline
				Backtracking & $O(k^n)$ & $O(n)$ \\
			\hline
				First Fit / Best Fit & $O(n \cdot m)$ & $O(n)$ \\
			\hline
				First Fit Decreasing / Best Fit Decreasing & $O(n \log n + n^2)$ & $O(n)$ \\
			\hline
		\end{tabular}
	\end{center}
	{Unde $n$ reprezintă numărul de pachete, $m$ numărul de containere, iar $k$ numărul de opțiuni de plasare}
	...
	Avantaje:
	\begin{itemize}
		\item garantează soluția optimă;
		\item poate genera toate soluțiile optime;
		\item pruning-ul reduce semnificativ timpul de execuție.
	\end{itemize}

	Dezavantaje:
	\begin{itemize}
		\item complexitate exponențială;
		\item timp mare de execuție pentru seturi mari;
		\item implementare mai complexă.
	\end{itemize}

	\subsubsection{First Fit / Best Fit}

	Avantaje:
	\begin{itemize}
		\item foarte rapide și ușor de implementat;
		\item potrivite pentru seturi mari de date;
		\item variantele Decreasing oferă rezultate bune în practică.
	\end{itemize}

	Dezavantaje:
	\begin{itemize}
		\item nu garantează soluția optimă;
		\item pot produce soluții suboptimale;
		\item sensibile la ordinea pachetelor.
	\end{itemize}

	\nocite{*} % include toate referințele din Bibliografie.bib
	\printbibliography
